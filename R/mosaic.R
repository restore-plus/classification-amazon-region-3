...
cube_to_rgb_mosaic <- function(cube,
  ...
  print("Saving mosaic object")
  saveRDS(mosaic, "data/derived/mosaic-cube.rds")

  mosaic_files <- dplyr::bind_rows(mosaic[["file_info"]])

  # Update files
  purrr::map(seq_len(nrow(mosaic_files)), function(file_idx) {
    restoreutils:::C_Raster_Set_Scale_Offset(
      filename = mosaic_files[file_idx,][["path"]],
      scale    = 0.004,
      offset   = 0
    )
  })
